...
import { signToken, checkPassword, hashPassword, requireAuth, politiqueAbonnementEspace } from '../lib/auth.js';
...
r.post('/login', (req, res) => {
  ...
  const abonnement = politiqueAbonnementEspace({ role: u.role, did: u.distributeur_id, pid: u.proprietaire_id });
  res.json({
    token: signToken(u),
    user: { id: u.id, username: u.username, fullName: u.full_name, role: u.role, distributeur, proprietaire, abonnement }
  });
});
...
r.get('/me', requireAuth, (req, res) => {
  ...
  const abonnement = politiqueAbonnementEspace(req.user);
  res.json({ id: u.id, username: u.username, fullName: u.full_name, role: u.role, distributeur, proprietaire, abonnement });
});
