...
import { getFournisseur } from '../lib/gateway.js';
...
// Résumé d'abonnement + montant mensuel de référence.
r.get('/abonnement', (req, res) => {
  ...
  res.json({ abonnement: abonnementResume(p), montantMensuel: montantMensuel(p) });
});
...
r.post('/initier', (req, res) => {
  try {
    const { mois, montant, methode, provider } = req.body || {};
    const fournisseur = getFournisseur(provider);
    const paiement = creerPaiementEnAttente({ proprietaireId: req.user.pid, montant, methode, mois, provider: fournisseur.nom });
    const { paymentUrl } = fournisseur.initier({ paiement, proprietaire: { id: req.user.pid }, baseUrl: baseUrl(req) });
    res.json({ ok: true, paiementId: paiement.id, token: paiement.token, montant: paiement.montant, mois: paiement.mois, provider: fournisseur.nom, paymentUrl });
  } catch (e) {
